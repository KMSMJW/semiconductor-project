...
def samsung_model(**kwargs):
    ...
    for l in range(0,PKG):
        for t in range(1,time):
            mdl.add_constraint(alpha_PKG[l,t] == 0.05 - (mdl.sum(eoh_PKG[j,l,t-1] for j in range(0,wafer))-625)/(625*7))
    for l in range(0,PKG):
        mdl.add_constraint(alpha_PKG[l,0] == 0.05 - (mdl.sum(PKG_inventory[j,l] for j in range(0,wafer))-625)/(625*7))
    ...
    for i in range(0,Fab):
        for t in range(0,time):
            mdl.add_constraint(mdl.sum(k[i,j,t] for j in range(0,wafer)) <= Capa_Fab_line[i])

    for i in range(0,Fab):
        for t in range(1,time):
            mdl.add_constraint(alpha_Fab[i,t] == 0.05 - (mdl.sum(eoh_Fab[i,j,t-1] for j in range(0,wafer)) - 1500)/(1500*7))

    for i in range(0,Fab):
        mdl.add_constraint(alpha_Fab[i,t] == 0.05 - (mdl.sum(avail_Fab_inventory[i,j]+non_avail_inventory[i,j] for j in range(0,wafer))-1500)/(1500*7))
...
df_result.to_excel("model1.xlsx",sheet_name='model1')
