...
for g in range(0,60):
    ...
    df8 = pd.read_excel('constant.xlsx',sheet_name='constant')
    Target = {}
    for j in range(0,wafer):
        for t in range(0,time):
    ...
    input_check = []
    inter_list = []
    fab_coef = df8.iloc[g,2]
    fab_inter = df8.iloc[g,3]
    pkg_coef = df8.iloc[g,4]
    pkg_inter = df8.iloc[g,5]
                    ...
                    for l in range(0, PKG):
                        ...
                        if JL[j, l] == 1:
                            if sum(mdl.z_in[j, l, k].solution_value for l in range(0, PKG)) == 0:
                                ...
                                rato[(j, l, t)] = mdl.z_in[j, l, k].solution_value / sum(
                                    mdl.z_in[j, l, k].solution_value for l in range(0, PKG))
            output = []
            check = []
            output1 = []
            regression = []
    ...
    result.append([g,fab_tat*5-60,pkg_tat*5-25,plan_fill,simu_fill])
result_columns = ['scenario', 'Fab', 'PKG', 'plan', 'implement']
df_result = pd.DataFrame(result, columns=result_columns)
df_result.to_excel("avail_con.xlsx", sheet_name='avail_con')