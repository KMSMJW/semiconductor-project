...
#생산 목표
df7 = pd.read_excel('Target.xlsx', sheet_name='50-2')
Target = {}
for j in range(0,wafer):
    for t in range(0,time):
        Target[(j,t)] = df7.iloc[j,t%12+1]
...
def samsung_model(**kwargs):
    ...
    for i in range(0,Fab):
        for j in range(0,wafer):
            for t in range(2,time):
                mdl.add_constraint(eoh_Fab[i,j,t] == eoh_Fab[i,j,t-1] + x_in[i,j,t-2] - x_out[i,j,t])
    for i in range(0,Fab):
        for j in range(0,wafer):
            mdl.add_constraint(eoh_Fab[i,j,1] == eoh_Fab[i,j,0] + F_in[i,j] - x_out[i,j,1])
    for i in range(0,Fab):
        for j in range(0,wafer):
            mdl.add_constraint(eoh_Fab[i,j,0] == Fab_inventory[i,j] + F_in[i,j] - x_out[i,j,0])
    ...
    for i in range(0,Fab):
        for t in range(0,time):
            for j in range(0,wafer):
                mdl.add_constraint(x_out[i,j,t] <= eoh_Fab[i,j,t])
...
df_result.to_excel("fixed.xlsx", sheet_name='fixed')
